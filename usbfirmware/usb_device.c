 ...
 *******************************************************************/
	...
	volatile BDT_ENTRY BDT[(USB_MAX_EP_NUMBER + 1) * 4] __attribute__ ((aligned (512))) @ USBRAM_PINGPONGBUFFER;
#endif
 ...
 *******************************************************************/
	...
	volatile CTRL_TRF_SETUP SetupPkt @ USBRAM_EP0_SETUPPKT;           // 8-byte only
	volatile BYTE CtrlTrfData[USB_EP0_BUFF_SIZE] @ USBRAM_EP0_BUFFER;
#else
...
// Can provide compile time option to do software pingpong
		...
		// So the variables here have to be mapped manually into USBRAM
		// or nothing will work...
	    volatile unsigned char hid_report_out[HID_INT_OUT_EP_SIZE] @ USBRAM_HIDOUT;
	    volatile unsigned char hid_report_in[HID_INT_IN_EP_SIZE] @ USBRAM_HIDIN;
	#else
